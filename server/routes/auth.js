...
// Passwordless sign-in codes ("magic link" as a 6-digit code — easier on phones).
let loginReady = null;
function ensureLoginTable() {
  if (!loginReady) {
    loginReady = exec(`CREATE TABLE IF NOT EXISTS login_codes (
      token      TEXT PRIMARY KEY,
      email      TEXT NOT NULL,
      code       TEXT NOT NULL,
      expires_at TEXT NOT NULL,
...
/** POST /api/auth/login-code { email } — email a 6-digit sign-in code. */
router.post('/login-code', async (req, res) => {
  await ensureLoginTable();
  const email = (req.body?.email || '').trim().toLowerCase();
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) return res.status(400).json({ error: 'Enter a valid email address' });
  if (!emailer.isEnabled()) return res.status(503).json({ error: 'Email sign-in is not set up yet. Please log in with your password.' });

  const existing = await get(`SELECT name, active FROM users WHERE lower(email) = ?`, [email]);
  if (existing && !existing.active) return res.status(403).json({ error: 'Account disabled. Contact admin.' });

  await run(`UPDATE login_codes SET used = 1 WHERE email = ? AND used = 0`, [email]);
  const code = String(crypto.randomInt(0, 1000000)).padStart(6, '0');
  const token = crypto.randomBytes(16).toString('hex');
  const expires = new Date(Date.now() + 10 * 60 * 1000).toISOString();
  await run(`INSERT INTO login_codes (token, email, code, expires_at) VALUES (?,?,?,?)`, [token, email, code, expires]);

  try {
    await emailer.sendEmail({
      to: email,
      subject: `${code} is your Kisan Sathi sign-in code`,
      html: emailer.loginCodeEmailHtml(existing ? existing.name : '', code),
    });
  } catch (e) {
    console.error('login code email failed:', e.message);
    return res.status(502).json({ error: 'Could not send the email right now. Please try again shortly.' });
  }
  res.json({ ok: true, message: 'We emailed you a 6-digit sign-in code.' });
});

/** POST /api/auth/login-code/verify { email, code } — sign in (creating a farmer if new). */
router.post('/login-code/verify', async (req, res) => {
  ...
  const row = await get(`SELECT * FROM login_codes WHERE email = ? AND used = 0 ORDER BY created_at DESC LIMIT 1`, [email]);
  if (!row || new Date(row.expires_at) < new Date()) {
    return res.status(400).json({ error: 'That code has expired. Please request a new one.' });
  }
  if (row.attempts >= 5) {
    await run(`UPDATE login_codes SET used = 1 WHERE token = ?`, [row.token]);
    return res.status(429).json({ error: 'Too many wrong attempts. Please request a new code.' });
  }
  if (String(row.code) !== otp) {
    await run(`UPDATE login_codes SET attempts = attempts + 1 WHERE token = ?`, [row.token]);
    return res.status(400).json({ error: `Incorrect code. ${4 - row.attempts} attempt(s) left.` });
  }
  await run(`UPDATE login_codes SET used = 1 WHERE token = ?`, [row.token]);
  ...
  if (!user) {
    // New email -> create a farmer account (same rule as Google sign-in).
    ...
    const info = await run(
      ...
      [email.split('@')[0], email, randomHash]
    );
